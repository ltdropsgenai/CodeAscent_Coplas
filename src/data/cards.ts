 ...
 * cards. All art is bundled in the binary (see data/cardImages, generated by
 * scripts/bundle-cards.mjs), so the full deck plays offline; only video streams.
 */
 ...
 *
 * Sorted on the NOUN, not the full name. Sorting on "El Barril" / "La Bandera"
 * buckets the whole deck by its article first — every El card lands before
 * every La card — so La Bandera came out #636 while El Barril was #55, which
 * looks like the numbers mean something they don't. Dropping the article gives
 * an actual A-Z deck.
 *
...
function renumber(deck: Card[]): Card[] {
  ...
  const noun = (name: string) => name.replace(/^(El|La|Las|Los)\s+/i, '');
  return [...deck]
    .sort((a, b) => collator.compare(noun(a.name), noun(b.name)) || a.id.localeCompare(b.id))
